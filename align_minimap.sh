...
#SBATCH --array=2-12 #e.g.--array=0,6,16-32, --array=0-15:4 (equivalent to --array=0,4,8,12), --array=0-15%4 (limit number of simultaneously running jobs) 
set -xv

start_time=`date --date='0 days ago' "+%Y-%m-%d %H:%M:%S"`

cd /home/maxine91/scratch/sorted_align_bam.dir
...
if [ $? -eq 0 ]; then
  echo "Alignment completes!"
else
  echo "alignment is failed."
fi
...
if [ $? -eq 0 ]; then
  echo "Converting completes!"
else
  echo "Converting is failed."
fi
...
echo "Sorting completes!"
if [ $? -eq 0 ]; then
  echo "Sorting completes!"
else
  echo "Sorting is failed."
fi
